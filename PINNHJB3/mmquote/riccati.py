...
B carries all bid/ask asymmetry (V_-, D_-), the price drift mu (Section 5), and
the fixed-cost aggregate V~_-; each defaults to zero.
"""
...
class ProxyCoefficients:
    """Per-asset Taylor coefficients of the two Hamiltonians and size moments.

    All arrays have shape (d,). For single-CUSIP, d = 1.

    Under distributed request sizes (Section 5 / Prop. 4), the Model-B alphas are
    z-independent and the Delta_{j,k} aggregates reduce to alpha_j * E[z^k]; hence
    only the first two size moments enter: z is read as m1 = E[z], and m2 = E[z^2]
    (defaulting to z**2, which reproduces the fixed-size model exactly).
    """
    ...
    alpha1_a: np.ndarray
    alpha2_a: np.ndarray
    z: np.ndarray                      # first size moment E[z] (equals z for point mass)
    m2: np.ndarray | None = None       # second size moment E[z^2]; defaults to z**2
    ...
    def __post_init__(self) -> None:
        ...
        if self.m2 is None:
            object.__setattr__(self, "m2", self.z**2)
        elif self.m2.shape != (d,):
            raise ValueError("m2 must share shape (d,).")
    ...
    @property
    def D_minus(self) -> np.ndarray:  # (alpha2_b - alpha2_a) * E[z^2], diagonal
        return np.diag((self.alpha2_b - self.alpha2_a) * self.m2)
...
def solve_riccati(
    ...
    V_tilde_minus: np.ndarray | None = None,
) -> RiccatiSolution:
    """Closed-form A(t) via matrix tanh; B(t) by backward integration; ergodic limits.

    mu            : price drifts (Section 5); enters the B ODE as an additive term.
    V_tilde_minus : fixed-cost aggregate V~_- (Prop. 5); zero when costless.
    """
    d = coeffs.z.shape[0]
    ...
    Vt = np.zeros(d) if V_tilde_minus is None else np.asarray(V_tilde_minus, dtype=float)
    ...
    for i in range(len(t_grid) - 2, -1, -1):
        A_next = A_path[i + 1]
        rhs = (mu + 2.0 * A_next @ (Vm + Vt + Dm @ np.diag(A_next))
               + 2.0 * A_next @ Dp @ B_path[i + 1])
    ...
    B_erg = -(Dp_ih @ Ahat_pinv @ Dp_h) @ mu - (Dp_ih @ Ahat @ Ahat_pinv @ Dp_ih) @ (
        Vm + Vt + 0.5 * np.sqrt(gamma) * Dm @ np.diag(Gamma)
    )
