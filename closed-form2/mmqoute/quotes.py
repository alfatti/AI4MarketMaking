...
class QuoteEngine:
    ...
    def marginal_values(self, q: np.ndarray, t: float = 0.0) -> tuple[float, float]:
        """(p_b, p_a) at inventory vector q; affine in q under the proxy."""
        A, B = self._AB(t)
    ...
    def quote(self, q: np.ndarray | float, t: float = 0.0) -> Quote:
        ...
        p_b, p_a = self.marginal_values(q, t)
        return Quote(
