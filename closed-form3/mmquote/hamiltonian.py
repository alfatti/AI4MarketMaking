...
def _solve_x(c: np.ndarray) -> np.ndarray:
    """Solve x - exp(-x) = c elementwise (strictly increasing, unique root).

    This is the FOC of the logistic Model-B Hamiltonian: with
    Lambda(delta) = lam/(1+e^{alpha+beta*delta}) and u = e^{alpha+beta*delta*},
    the optimizer satisfies x - e^{-x} = 1 + alpha + beta*p where x = log u.
    """
    c = np.asarray(c, dtype=float)
    x = np.where(c >= 0.0, c, -np.log1p(-np.minimum(c, 0.0)))   # robust init on both tails
    for _ in range(25):
        e = np.exp(np.clip(-x, -700.0, 700.0))
        x = x - (x - e - c) / (1.0 + e)
    return x
...
class HamiltonianSide:
    ...
    # ----- semi-closed path (logistic intensity, Model B only) -----

    def _logistic_fast(self) -> bool:
        return isinstance(self.intensity, LogisticIntensity) and self.xi == 0.0

    def _log_x(self, p) -> np.ndarray:
        lam = self.intensity
        return _solve_x(1.0 + lam.alpha + lam.beta * np.asarray(p, dtype=float))
    ...
    def H(self, p: np.ndarray | float) -> np.ndarray | float:
        ...
        if self._logistic_fast():
            lam = self.intensity
            x = self._log_x(p)
            return (lam.lam_rfq / lam.beta) * np.exp(np.clip(-x, -700, 700))
    ...
    def dH(self, p: np.ndarray | float) -> np.ndarray | float:
        if self._exp_params() is not None:
            _, k = self._exp_params()
            return -k * self.H(p)
        if self._logistic_fast():                       # envelope: H'(p) = -Lambda(delta*)
            lam = self.intensity
            x = self._log_x(p)
            return -lam.lam_rfq / (1.0 + np.exp(np.clip(x, -700, 700)))
    ...
    def d2H(self, p: np.ndarray | float) -> np.ndarray | float:
        ...
        if self._logistic_fast():
            lam = self.intensity
            x = self._log_x(p)
            ex = np.exp(np.clip(x, -350, 350))
            return lam.lam_rfq * lam.beta * ex**2 / (1.0 + ex) ** 3
    ...
    def delta_star(self, p: np.ndarray | float) -> np.ndarray | float:
        ...
        if self._logistic_fast():
            lam = self.intensity
            return (self._log_x(p) - lam.alpha) / lam.beta
        if self.xi == 0.0:
            target = -self.dH(p)
        else:
